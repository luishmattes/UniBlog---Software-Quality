...
export const createPostSchema = z.object({
  ...
  content_Post: z.string().min(1, 'O conteúdo não pode estar vazio').max(1000, 'O conteúdo pode ter no máximo 1000 caracteres'),
  image_Post: z.string().optional(),
  id_Perfil_Post: z.string().uuid('ID de perfil inválido').optional(),
});

export const updatePostSchema = z.object({
  id_Post: z.string().uuid('ID de post inválido'),
  title_Post: z.string().max(100, 'O título pode ter no máximo 100 caracteres').optional(),
